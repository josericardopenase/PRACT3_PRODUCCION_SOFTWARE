...
def test_remove_expense_reduces_total():
    ...
    service.create_expense(title="Libro", amount=3, description="", expense_date=date.today())
    service.create_expense(title="Revista", amount=4, description="", expense_date=date.today())
    
    expenses = service.list_expenses()
    first_expense = expenses[0].id
    service.remove_expense(first_expense)
    actual_expenses = service.list_expenses()

    assert len(actual_expenses) == 1
    assert actual_expenses[0].title == "Revista"
...
def test_update_expense_partial_fields():
    ...
    update_expense = service.list_expenses()[0]

    assert update_expense.title == "Camiseta"
    assert update_expense.amount == 18
    assert update_expense.description == "Ropa"
