...
def test_get_by_id_returns_expense():
    ...
    """
    repo = InMemoryExpenseRepository()
    expense = create_expense(id=1, title="Get", amount=10)
    repo.save(expense)
    expected_expense = repo.get_by_id(1)

    assert expected_expense is not None
    assert expected_expense.id == 1
    assert expected_expense.title == "Get"
    assert expected_expense.amount == 10
...
def test_get_by_id_returns_none_if_not_found():
    ...
    repo.save(expense)
    expected_expense = repo.get_by_id(5)
    
    assert expected_expense is None
