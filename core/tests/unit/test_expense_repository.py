...
def test_get_by_id_returns_expense():
    ...
    expense = create_expense()
    repo.save(expense)
    same_expense = repo.get_by_id(expense_id=1)
    assert same_expense is not None
    assert same_expense.id == expense.id
    assert same_expense.title == expense.title
    assert same_expense.amount == expense.amount
...
def test_get_by_id_returns_none_if_not_found():
    ...
    """
    repo = InMemoryExpenseRepository()
    expense = repo.get_by_id(expense_id=999)
