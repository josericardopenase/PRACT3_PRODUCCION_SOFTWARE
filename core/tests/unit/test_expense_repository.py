...
def test_get_by_id_returns_expense():
    ...
    """
    repo = InMemoryExpenseRepository()
    expense = create_expense(id=1, title="Test Búsqueda", amount=33)
    repo.save(expense)

    retrieved_expense = repo.get_by_id(1)

    assert retrieved_expense is not None
    assert retrieved_expense.id == 1
    assert retrieved_expense.title == "Test Búsqueda"
    assert retrieved_expense.amount == 33
...
def test_get_by_id_returns_none_if_not_found():
    ...
    retrieved_expense = repo.get_by_id(13)
