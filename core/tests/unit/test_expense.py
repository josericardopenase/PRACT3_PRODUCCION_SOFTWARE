...
def test_negative_amount_raises_error():
    with pytest.raises(InvalidAmountError):
        Expense(id=1, title="Transporte", amount=-40, description="Uber", expense_date=date.today())


def test_future_date_raises_error():
    with pytest.raises(InvalidExpenseDateError):
        Expense(id=1, title="Ropa", amount=39.99, description="Sudadera Nike", expense_date=date.today()+timedelta(days=1))
