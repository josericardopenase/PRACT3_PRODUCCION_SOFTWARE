...
def test_negative_amount_raises_error():
    ...
    with pytest.raises(InvalidAmountError):
        Expense(id=1, title="Maquillaje", amount=-10, description="", expense_date=date.today())
...
def test_future_date_raises_error():
    ...
    with pytest.raises(InvalidExpenseDateError):
        Expense(id=1, title="Viaje", amount=200, description="", expense_date=date.today()+timedelta(days=1))
